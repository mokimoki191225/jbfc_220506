...
def find_books_of_mine(db_name, fillter):
    ...
    try:
        ...
        db_sql += "WHERE title like '%{}%'" .format(fillter) # <-문자는 싱글코테이션, 밖은 더블코테이션

        #명령어를 실행하는 execute이다.
        cur.execute(db_sql)

        # 조회한 데이터 불러오기
        print('[6] 내가 만든 조건 책 출력하기')
        books = cur.fetchall()
...
if __name__ == '__main__':
    ...
    is_success, books_df = find_books_of_mine(db_name, fillter)

    if is_success:
        print('조건에 맞는 데이터는 총 %d건 입니다. \n (조건:사물글자들어간거)' %len(books_df))
    else:
