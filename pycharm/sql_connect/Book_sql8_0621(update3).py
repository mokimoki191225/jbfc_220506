...
def update_books(db_name):
    ...
    try:
        ...
        # 조회한 데이터 불러오기
        print('[8] 데이터 수정된거불러오기')
        book = cur.fetchone()
        books = [book]
        ret_df2 = getBooksDF(books)
    ...
    return is_success, ret_df2

if __name__ == '__main__':

    db_name = 'bigpy'

    is_success, ret_df2 = update_books(db_name)

    print(ret_df2)

    is_success, ret_df = one.select_one_books(db_name)
    ...
    # books_df = pd.concat([ret_df,ret_df2],axis=0)
