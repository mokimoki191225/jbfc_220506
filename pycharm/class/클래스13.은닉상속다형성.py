...
for x in range(10):
    ...
    time.sleep(1)
    marine.get_enrgy(); time.sleep(1)
    corsair.get_enrgy(); time.sleep(1)
    hydra.get_enrgy(); time.sleep(1)

    if (int(marine.is_alive) + int(corsair.is_alive) + int(hydra.is_alive))==1:
        time.sleep(1)
    else:
        break
